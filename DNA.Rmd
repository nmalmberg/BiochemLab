...
```{r gelimage, fig.cap='(text:gelimage)', out.width="100%"}
knitr::include_graphics("GelImage.JPG",dpi=NA)
```

(text:gelimage) Make sure you describe the contents of each lane in this figure caption, assuming you are planning to include the figure.
...
```{r alignment, fig.cap="(text:alignment)"}
...
for (primer in levels(omrf$Primer)){
  referencefile <- paste(primer, ".fasta")
  if (file.exists(referencefile)){
    ...
    for (sequencefile in sequencefiles){
      sequences[sequencefile] <- paste0(readLines(sequencefile), collapse = "")
    }
    # Assume reference sequence is in FASTA format, so the first line is skipped.
    referenceseq <- paste0(readlines(referencefile)[-1], collapse="")
    Biostrings::pairwiseAlignment(pattern=sequences, subject=referenceseq)
  }
}
```

(text:alignment) Your caption describing the alignments should go here.
